...
run one, and you get the figure that appears on the matching gallery page.
...
## No shared helper module, on purpose

An earlier version of this gallery had one, and it was a mistake. A private
helper is the worst of both worlds: a library the reader cannot install but
still has to read in order to follow the example importing it. The few lines
that would live in it are the lesson in most cases, so they are repeated
inline instead, and the repetition reinforces rather than pads.

The cost is that the endpoint appears in every file, so
`scripts/build_gallery.py --check` verifies the `S3 = {...}` line is
identical across all of them and names the odd one out.

If a client package ever lands, these become its showcase and every one of
them gets shorter.
...
`scripts/build_gallery.py` does the translation. It never edits a script, so
...
biggest lever anybody has over how long an analysis takes, so the gallery
...
3. Keep it small. The whole gallery runs in a couple of minutes because no
...
4. Run it and render the page:
   ```bash
   cd ..
   python scripts/build_gallery.py --run --only 09
   make docs
   ```
5. Add the page to the `nav:` block in `mkdocs.yml`. The strict build fails
   with a clear message if you forget.
6. Commit the figure and thumbnail in `assets/gallery/` with the updated
   `gallery.lock`. Figures are committed on purpose: the docs build stays
...
`python scripts/build_gallery.py --check` compares each script against the
...
To refresh them, run `make gallery` locally, or trigger the `gallery`
