...
PAGE_NAME = "databrowser.md"
MOUNT_MARKER = "<!-- databrowser-mount -->"
LOG_TAG = "databrowser"
...
def render_mount(entry_js: str, inspector_js: str) -> str:
    """The mount div that replaces the marker. Nothing else on the page.

    The URLs are the only reason this is generated at all: npm emits file
    names with content hashes in them, so they are not knowable until the
    ...
    return f"""<div class="waterpark-databrowser-wrapper">
...
def main() -> None:
    ...
    inject_mount(render_mount(entry_js, inspector_js))
