...
#: The hand-written page and the marker this script replaces in it.
PAGE_NAME = "stac-browser.md"
MOUNT_MARKER = "<!-- stac-browser-mount -->"
LOG_TAG = "stac-browser"
...
def render_mount(entry_js: str, entry_css: list[str]) -> str:
    """The mount div that replaces the marker. Nothing else on the page.

    The URLs are the only reason this is generated at all: the bundler emits
    file names with content hashes in them, so they are not knowable until
    ...
    return f"""<div class="waterpark-stac-wrapper">
...
def inject_mount(markup: str) -> None:
    ...
    """
    page = BUILD_DATA / PAGE_NAME
    if not page.exists():
        raise SystemExit(
            f"[{LOG_TAG}] {page} is missing. The page is hand-written: create "
            f"data/{PAGE_NAME} and put {MOUNT_MARKER} where the app goes."
        )

    text = page.read_text()
    if MOUNT_MARKER not in text:
        raise SystemExit(
            f"[{LOG_TAG}] no {MOUNT_MARKER} in data/{PAGE_NAME}, so there is "
            "nowhere to mount the app. Add the marker on a line of its own."
        )

    page.unlink()  # never write through the serve symlink
    page.write_text(text.replace(MOUNT_MARKER, markup))
...
def main() -> None:
    ...
    inject_mount(render_mount(entry_js, entry_css))
