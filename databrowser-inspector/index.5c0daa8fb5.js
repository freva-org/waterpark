const V = `<svg viewBox="0 0 448 512" width="10" height="10" style="vertical-align:middle;">
...
</svg>`, Z = `<svg viewBox="0 0 256 512" width="6" height="10" style="vertical-align:middle;">
...
class pt extends HTMLElement {
  constructor() {
    super(...arguments), this._config = { ...H }, this._showAdvanced = !1, this._handleChange = (t) => {
      ...
      if (t.target.closest("#nc-advanced-toggle")) {
        ...
        e && (e.style.display = this._showAdvanced ? "block" : "none"), i && (i.innerHTML = this._showAdvanced ? V : Z);
      }
    };
  }
  connectedCallback() {
    ...
    this.innerHTML = `
      <div class="aggregation-config">
        ...
        <button type="button" id="nc-advanced-toggle"
          ...
          <span class="nc-chevron">${this._showAdvanced ? V : Z}</span>
...
function mt() {
  ...
  try {
    if (typeof CSSStyleSheet == "function" && Array.isArray(document.adoptedStyleSheets)) {
      ...
      t.replaceSync(Y), document.adoptedStyleSheets = [...document.adoptedStyleSheets, t];
      return;
    }
  } catch {
  }
  const r = document.createElement("style");
  r.setAttribute("data-data-inspector", "1"), r.textContent = Y, document.head.appendChild(r);
}
class ft extends HTMLElement {
  constructor() {
    super(...arguments), this._pathInput = "", this._copied = !1, this._gridlookCopied = !1, this._activeTab = "metadata", this._dropdownOpen = !1, this._aggregationConfig = { ...ut }, this._output = null, this._built = !1, this._builtMode = !1, this._domOutput = void 0, this._domIframeUrl = void 0, this._domFileKey = void 0, this._copyTimer = null, this._gridlookTimer = null, this._restoreFocusTo = null, this._onOutsideClick = (t) => {
  ...
  _attach() {
    ...
    t && (t.value = this._pathInput, t.addEventListener("input", (c) => {
      ...
      const c = this.zarrUrl;
      c && window.open(F(c), "_blank", "noopener,noreferrer");
  ...
  _update() {
    ...
    if (t) {
      const x = this._q("#nc-file-list-wrap"), A = d.length > 0;
      if (this._toggle(x, A), A) {
    ...
    if (!t) {
      const x = this._q("#nc-load-btn"), A = this._q("#nc-load-toggle");
      x && (x.disabled = i), A && (A.disabled = i), this._syncDropdown();
    }
    const _ = !!s && s !== (typeof n == "string" ? n : null);
    if (this._toggle(this._q("#nc-zarr-row"), _), _ && s) {
      const x = this._q("#nc-zarr-url");
      x && (x.textContent = s);
    }
    ...
    if (c) {
      const x = this._activeTab === "metadata";
      c.classList.toggle("di-tab-active", x), c.setAttribute("aria-selected", String(x));
    }
    if (k) {
      const x = this._activeTab === "gridlook";
      k.disabled = e !== M.READY || !l, k.classList.toggle("di-tab-active", x), k.setAttribute("aria-selected", String(x));
    }
    ...
    if (this._toggle(this._q("#nc-gridlook"), C), s) {
      const x = F(s), A = this._q("#nc-gridlook-url");
      if (A && (A.textContent = x), C) {
        const I = this._ensureIframe();
        I && this._domIframeUrl !== x && (this._setIframeSrc(I, x), this._domIframeUrl = x);
      }
    }
...
class vt extends HTMLElement {
  ...
  connectedCallback() {
    xt(), this._render(), this._startTimers();
  }
  disconnectedCallback() {
    this._stopTimers();
  }
...
for (const [r, t] of [
  ["data-inspector", ft],
  ["aggregation-config", pt],
  ["zarr-loading-steps", vt]
])
...
export {
  pt as AggregationConfigElement,
  ft as DataInspectorElement,
  M as NcDumpDialogState,
  vt as ZarrLoadingStepsElement,
  qt as ZarrPoller,
